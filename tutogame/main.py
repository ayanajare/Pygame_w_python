import pygame  #2 d graphics library
import os #operation system (to define the path of the images)
pygame.font.init()
pygame.mixer.init()

WIDTH, HEIGHT = 900,500
WIN = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("piou piou game!")
...
RED_SPACESHIP = pygame.transform.rotate(
    pygame.transform.scale(RED_SPACESHIP_IMAGE, (SPACESHIP_WIDTH, SPACESHIP_HEIGHT)), 270) #resize and rotate
#background
...
def yellow_handle_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_q] and yellow.x - VEL> 0 :  # LEFT
        ...
        yellow.x += VEL
    elif keys_pressed[pygame.K_z] and yellow.y - VEL> 0:  # UP
        yellow.y -= VEL
    elif keys_pressed[pygame.K_s] and yellow.y + VEL + yellow.height< HEIGHT -15:  # DOWN
        yellow.y += VEL

def red_handle_movement(keys_pressed, red):
    if keys_pressed[pygame.K_LEFT] and red.x +VEL >BORDER.x + BORDER.width: # LEFT and
        red.x -= VEL
...
def draw_window(red, yellow, red_bullets, yellow_bullets,red_health, yellow_health) :
    ...
    WIN.blit(YELLOW_SPACESHIP, (yellow.x,yellow.y))#to show our text or images in the frame at a specific place
